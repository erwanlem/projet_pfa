...
let alexandre_pattern alexandre _ = 
  ...
  if Vector.dist playerpos (alexandre#pos#get) < 50.0 && alexandre # cld # get = 0 then
    ( if (State.get_state alexandre#state#get) = 0 then
        (let i = ref (-1) in
        alexandre # cld # set 20;
...
let alexandre_call alexandre () : unit =
  if not (alexandre # alive) then  
    ...
    if alexandre # cld # get > 0 then alexandre # cld # set (alexandre # cld # get -1);
    let playerpos = (Global.ply())#pos#get in
